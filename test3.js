...
updateMovieTicketInfo(sessionId,info);
  /*generateQuestion(obj);*/
    ...
    function cancel(agent)
    {
      ...
      agent.add(new dfff.Payload(agent.UNSPECIFIED,cancconfirm,{
        sendAsMessage: true, rawPayload: true}))
    }
    function cancyes(agent) 
    {
      agent.add("Sorry to see you go :(")  
    }
    function cancno(agent)
    {
      agent.add("okay, then let's proceed!")
    }
    ...
    intentmap.set("cancel yes", cancyes);
    intentmap.set("cancel no", cancno);
